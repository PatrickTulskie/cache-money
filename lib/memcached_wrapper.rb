...
Rails.logger.info("cache-money: Memcached installed") if defined? Rails
  ...
  class MemcachedWrapper < ::Memcached
    ...
    def add(key, value, ttl=@default_ttl, raw=false)
      ...
      log_error($!) if logger
      not_stored_or_server_unavailable(e)
    end
    ...
    # This is not a standard memcached response.  This will be used to detect when the server is not there so we can fail through to ActiveRecord
...
####### they have MemCache installed (don't need the wrapper)
  ...
  MemcachedWrapper = Class.new(MemCache)
